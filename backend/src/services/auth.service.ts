...
export class AuthService {
  ...
  static async sendOtp(phone: string) {
    ...
    const rateLimit = checkRateLimit(`send_otp_${phone}`, 5, 15 * 60 * 1000);
  ...
  static async register(input: any, ipAddress: string = "127.0.0.1", userAgentStr: string = "Unknown") {
    ...
    const emailAddr = input.email
      ...
      : `${input.username ? input.username.toLowerCase() : "user_" + Date.now()}@onevriksh.internal`;
    ...
    if (input.phone) {
      ...
      await sendSmsOtp(input.phone, otp);
    }
