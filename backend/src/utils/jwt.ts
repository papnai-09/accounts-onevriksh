...
function getAccessSecret(): Uint8Array {
  const secret = process.env.JWT_ACCESS_SECRET || "onevriksh_access_secret_change_in_production_32chars";
  return new TextEncoder().encode(secret);
}

function getRefreshSecret(): Uint8Array {
  const secret = process.env.JWT_REFRESH_SECRET || "onevriksh_refresh_secret_change_in_production_32chars";
  return new TextEncoder().encode(secret);
}
